...
def _install_fake_aliyun_modules(responses_by_instance: dict[str, dict], recorded_requests: list[dict]):
    class FakeAcsClient:
        ...
        def do_action_with_exception(self, request):
            ...
            return json.dumps(responses_by_instance[instance_id])
...
@pytest.mark.parametrize(
    ("db_type", "instance_id", "response", "expected_metrics", "expected_key_parts", "unexpected_key_parts"),
    [
        (
            "mysql",
            "rm-mysql-001",
...
async def test_aliyun_rds_template_supports_multiple_engines(
    ...
    recorded_requests = []
    fake_modules = _install_fake_aliyun_modules({instance_id: response}, recorded_requests)

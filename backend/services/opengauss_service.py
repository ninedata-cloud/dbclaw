...
class OpenGaussConnector(DBConnector):
    ...
    async def get_top_sql(self, limit: int = 100) -> List[Dict[str, Any]]:
        """Get TOP SQL statistics from pg_stat_statements (openGauss compatible)."""
        conn = await self._connect()
        try:
            try:
                rows = await conn.fetch(f"""
                    SELECT
                        ...
                        calls as exec_count,
                        ROUND(total_time / 1000, 6) as total_time_sec,
                        rows as total_rows_scanned,
                        0 as total_wait_time_sec,
                        ROUND(mean_time / 1000, 6) as avg_time_sec,
                        ROUND(rows::numeric / GREATEST(calls, 1), 2) as avg_rows_scanned,
                        0 as avg_wait_time_sec,

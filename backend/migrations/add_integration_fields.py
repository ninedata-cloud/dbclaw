...
async def migrate():
    ...
    async with async_session() as db:
        ...
        print("1. 添加 integration_id 字段到 integration 表...")
        ...
        # 添加 config_schema 字段
        print("3. 添加 config_schema 字段到 integration 表...")
        await db.execute(text("""
            ALTER TABLE integration
            ADD COLUMN IF NOT EXISTS config_schema JSON
        """))

        # 添加 channel_ids 字段到 alert_subscription
        print("4. 添加 channel_ids 字段到 alert_subscription 表...")
        await db.execute(text("""
            ALTER TABLE alert_subscription
            ADD COLUMN IF NOT EXISTS channel_ids JSON DEFAULT '[]'
        """))
        ...
        # 删除旧的 config 字段（如果存在）
        print("6. 删除 integration 表的旧 config 字段...")
        await db.execute(text("""
            ALTER TABLE integration

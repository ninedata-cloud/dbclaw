...
async def migrate():
    async with engine.begin() as conn:
        binding_exists_result = await conn.execute(text(
            ...
            "WHERE table_schema = current_schema() AND table_name = 'chat_channel_binding')"
        ))
        ...
        if not binding_exists:
            ...
            """))
            await conn.execute(text("CREATE INDEX ix_chat_channel_binding_channel_type ON chat_channel_binding(channel_type)"))
            await conn.execute(text("CREATE INDEX ix_chat_channel_binding_external_chat_id ON chat_channel_binding(external_chat_id)"))
            await conn.execute(text("CREATE INDEX ix_chat_channel_binding_external_user_id ON chat_channel_binding(external_user_id)"))
            await conn.execute(text("CREATE INDEX ix_chat_channel_binding_session_id ON chat_channel_binding(session_id)"))

"""
Migration: Rename connection_id to datasource_id in report table
"""
from sqlalchemy import text


def migrate(connection):
    """Rename connection_id to datasource_id in report table"""
    ...
    result = connection.execute(text("""
        ...
        WHERE table_name = 'report' AND column_name = 'datasource_id'
    """))
    if result.fetchone():
    ...
    result = connection.execute(text("""
        SELECT column_name FROM information_schema.columns
        WHERE table_name = 'report' AND column_name = 'connection_id'
    """))
    if not result.fetchone():
        print("Reports table does not have connection_id column, skipping migration")
        return

    print("Migrating report table: connection_id -> datasource_id")
    connection.execute(text("""
        ALTER TABLE report RENAME COLUMN connection_id TO datasource_id
    """))

...
Web 控制台支持简体中文（`zh-CN`）和英文（`en-US`），首次访问固定使用简体中文。登录前的选择保存在浏览器 `localStorage`；登录后的选择通过 `PUT /api/auth/me/locale` 保存到账号，并在后续登录和其他设备上生效。切换语言会保留当前 URL 和登录状态；编辑器或表单存在未保存内容时会先请求确认。

控制台 API 请求携带 `X-DBClaw-Locale`。服务端按请求头、账号偏好、`Accept-Language`、简体中文的顺序选择响应语言，并返回 `Content-Language`。错误响应继续保留兼容的 `detail` 字段，同时提供稳定错误码和插值参数：
...
AI 对话会将账号语言作为目标输出语言传给模型；手动触发的巡检报告同样使用当前账号语言，定时或告警自动触发的报告默认使用简体中文。AI 告警结论跟随自然语言告警规则的语言。SQL、命令、标识符、用户输入、厂商技术详情及引用原文不会被强制翻译；通知、公开分享和导出内容保持其生成时的语言。
...
`test:i18n` 校验中英文词典键、插值占位符和英文词条；Playwright 测试验证首次访问、账号偏好、未保存内容保护及主要控制台路由的双语言渲染。生产运行不依赖 Node.js 或 Playwright。
